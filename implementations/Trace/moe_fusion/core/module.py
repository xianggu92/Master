...
class multiTimeAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, dropout=None):
        ...
        dim = value.size(-1) # 60 = 30 * 2
        d_k = query.size(-1) # embed_time_k
        scores = torch.matmul(query, key.transpose(-2, -1)) \
                 / math.sqrt(d_k) # [Batch_size, h, seq_len, seq_len]  [32, 8, 48, 120]
        scores = scores.unsqueeze(-1).repeat_interleave(dim, dim=-1)
        if mask is not None: 
            if len(mask.shape)==3:
                mask=mask.unsqueeze(-1) # [Batch_size, 1,120, 60]

            scores = scores.masked_fill(mask.unsqueeze(-3) == 0, -10000) # [32, 8, 48, 120, 60]
    ...
    def forward(self, query, key, value, mask=None, dropout=0.1):
        "Compute 'Scaled Dot Product Attention'"
        batch, seq_len, dim = value.size() # (Batch_size, seq_len, input_dim)
        if mask is not None:
            # Same mask applied to all h heads.
            mask = mask.unsqueeze(1)
        value = value.unsqueeze(1)
        query, key = [l(x).view(x.size(0), -1, self.h, self.embed_time_k).transpose(1, 2)
                      for l, x in zip(self.linears, (query, key))]       # l1 for query, l2 for key
        x, _ = self.attention(query, key, value, mask, dropout) # [B, heads, tq=48, feature_dim=60]
